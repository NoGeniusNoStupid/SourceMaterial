   ...
   public class PageBarHelper
    {
       public static string GetPagaBar(int pageIndex, int listCount,int pageSize=10,int barCount=5)
       {
           ...
           //int start = pageIndex - barCount;//计算起始位置.要求页面上显示pageSize个数字页码.
           //if (start < 1)
           //{
           //    start = 1;
           //}
           ...
           //int end = Convert.ToInt32(Math.Ceiling(pageCount*1.0 / pageSize));//求记录数
           ...
           for (int i = start; i <= end; i++)
           {
               ...
               {
                   sb.AppendFormat("<a href='?pageIndex={0}' target='mainFrame' onfocus='this.blur()' >{0}</a>&nbsp;&nbsp;&nbsp;&nbsp;", i);
               }
           }
           //下一页
